...
const BlogCard = ({ blogImg, title, author, summary, id }) => {
  return (
    <>
      <Container style={"!items-start mb-10 !h-72 !justify-between"}>
        <Link href={`/blogs/${id}`}>
          <Image src={blogImg} width={200} height={200} alt="" className="rounded-lg max-h-36" />
        </Link>
        <Link href={`/blogs/${id}`}>
        ...
        <div>
          <div
            className="text-gray-600 text-sm group-hover:text-white line-clamp-3"
            dangerouslySetInnerHTML={{
              __html: summary.length > 150 ? summary.split(0, 25) + "..." : summary,
            }}
          ></div>
        </div>
